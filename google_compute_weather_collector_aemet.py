...
# This script is meant to be run on a Google Compute instance, daily,
# with a crontab command
...
# Note: In the same folder where this script will be, there also needs to be
# a Google Service Account key in a json file (with Google Storage permissions)
...
def upload_dfs_to_google_storage(dict_dfs_weather, bucket_name):
    ...
    for city, df in dict_dfs_weather.items():
        ...
        file_name = 'GoogleCompute/' + city + '/' + the_year + '/' + the_month + '/' + the_day + '.csv'
